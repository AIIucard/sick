...
public class ComplianceNodeManager extends NodeManagerUaNode {
  ...
  private void createAccessLevelVariable(String name, EnumSet<AccessLevel> accessLevel,
      EnumSet<AccessLevel> userAccessLevel) throws StatusException {
    final NodeId nodeId = new NodeId(getNamespaceIndex(), name);

    // just some datatype
    UaType type = getServer().getNodeManagerRoot().getType(Identifiers.Int32);
  ...
  private void createAnalogItem(AnalogData a) throws StatusException, UaInstantiationException, NodeBuilderException {
    ...
    conf.addOptional(n2e(Identifiers.AnalogItemType_EngineeringUnits));
    conf.addOptional(n2e(Identifiers.DataItemType_Definition));
    if (a.getInstrumentRange() != null) {
      conf.addOptional(n2e(Identifiers.AnalogItemType_InstrumentRange));
    }
  ...
  private void initialize() throws StatusException, UaInstantiationException, NodeBuilderException {
    ...
    // AccessLevel nodes
    createAccessLevelVariable("AccessLevelCurrentReadWrite", AccessLevel.READWRITE, AccessLevel.READWRITE);
    createAccessLevelVariable("AccessLevelCurrentRead", AccessLevel.READONLY, AccessLevel.READONLY);
    createAccessLevelVariable("AccessLevelCurrentWrite", AccessLevel.WRITEONLY, AccessLevel.WRITEONLY);
    createAccessLevelVariable("AccessLevelCurrentReadNotUser", AccessLevel.READONLY, AccessLevel.NONE);
    createAccessLevelVariable("AccessLevelCurrentWriteNotUser", AccessLevel.WRITEONLY, AccessLevel.NONE);
  ...
  private void initNode(UaVariable node, CommonComplianceInfo info, FolderType parent) throws StatusException {
    EnumSet<WriteAccess> set =
        WriteAccess.getSupportedWriteAccess(AttributeUtil.getSupportedAttributes(node.getNodeClass()));
    // Compiance test tool requires writing attributes

    // removing these as the may break something with ctt
    set.remove(WriteAccess.NodeId);
    set.remove(WriteAccess.NodeClass);
    set.remove(WriteAccess.ArrayDimensions);
    set.remove(WriteAccess.AccessLevel);
    set.remove(WriteAccess.UserAccessLevel);
    set.remove(WriteAccess.BrowseName);
    set.remove(WriteAccess.DataType);
    set.remove(WriteAccess.WriteMask);
    set.remove(WriteAccess.UserWriteMask);

    node.setWriteMask(set);
    node.setUserWriteMask(set);
