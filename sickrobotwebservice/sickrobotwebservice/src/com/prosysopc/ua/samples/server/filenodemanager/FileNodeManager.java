...
public class FileNodeManager extends NodeManagerUaNode {
  ...
  public UaNode addNode(UaNode newNode) throws StatusException {
    ...
    if (allowWrite && (node instanceof FileTypeNode)) {
      BaseNode.setAccessLevelRecursively(node, AccessLevel.READWRITE, Identifiers.HasComponent,
          Identifiers.HasProperty);
      ((FileTypeNode) node).setWritable(Boolean.TRUE);
    }
    if (node instanceof FileTypeNode) {
      EnumSet<AccessLevel> s = ((FileTypeNode) node).getSizeNode().getAccessLevel();
      EnumSet<AccessLevel> ns = EnumSet.copyOf(s);
      ns.add(AccessLevel.SemanticChange);
      ((FileTypeNode) node).getSizeNode().setAccessLevel(ns);
    }
