 ...
 * <p>
 * OPC UA enum types are expected to be zero-based, i.e. the first element is supposed to correspond
 * to 0. getEnumStrings() here is coded so that it will return null values, though, if not all
 * integer in between values are defined.
 * <p>
 * Note that it is usually easier to define new data types and use them with the information model
 * files and code generation, based on those.
 * <p>
 * The various {@link #valueOf} methods are useful for converting integer values to the enumeration.
 * In OPC UA communication, the enumeration values are always passed as integers.
 */
