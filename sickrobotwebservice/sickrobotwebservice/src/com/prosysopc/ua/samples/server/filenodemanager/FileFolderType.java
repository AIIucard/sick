...
/**
 * Implementation of a FolderType that contains files, i.e. maps to a Directory
 * on a physical disk.
 */
...
public class FileFolderType extends FileDirectoryTypeNode {
	 ...
	 * @param nodeManager
	 * @param file
	 * @return the BrowseName corresponding to the file. It equals to the name of
	 *         the file in the namespace of nodeManager
	 */
	 ...
	 * @param nodeManager
	 * @param fileName
	 * @return the BrowseName corresponding to the fileName. It equals to the
	 *         fileName in the namespace of nodeManager
	 */
	...
	/**
	 * Defines a "standard" mapping between file names and NodeIds, used internally
	 * by the manager to create IDs for new files.
	 * 
	 * @param nodeManager
	 * @param file
	 *            the file
	 * @return the nodeId corresponding to the file. The NodeId is composed of the
	 *         file path in the namespace of nodeManager
	 */
	...
	/**
	 * Add the file as a component to the folder. Creates a new {@link FileTypeNode}
	 * - or FileFolderType, if the file is a directory.
	 * 
	 * @param file
	 *            the file to add
	 * @return true if the file was added
	 * @throws StatusException
	 *             with Bad_NotWritable, if the file cannot be written, even though
	...
	public boolean addFile(File file) throws StatusException {
		...
		if (file.isDirectory()) {
			...
			if (node == null) {
				FileTypeNode fileType = nodeManager.createInstance(FileTypeNode.class, idForFile,
						browseNameForFile(file), displayNameForFile(file));
	 ...
	 *            the node to add
	 * @param createFile
	 *            if true, the file is created if it does not exist (along with the
	 *            folder path to the file)
	 * @return true if the file was added
	 * @throws StatusException
	 *             with Bad_NotWritable, if the file cannot be written, event though
	 *             {@link #isWritable()} is set.
	 */
	 ...
	 * @param file
	 * @return the BrowseName corresponding to the file. It equals to the name of
	 *         the file in the namespace of this nodeManager
	 */
	 ...
	 * @param fileName
	 * @return the BrowseName corresponding to the fileName. It equals to the
	 *         fileName in the namespace of this nodeManager
	 */
	...
	/**
	 * @param fileName
	 * @return the DisplayName corresponding to the fileName. It equals to the
	 *         fileName with LocalizedText.NO_LOCALE
	 */
	...
	/**
	 * If files cannot be added to monitoring in {@link #refresh()} they will be
	 * added to the failed files. A logger warning is also generated for them in the
	 * first failure. Remove the file from the list to make the manager retry adding
	 * it.
	 * 
	 ...
	 * <p>
	 * The file name is checked if it contains any of the comma delimited patterns
	 * in the filter. Each pattern may begin or start with * as a wildcard, in which
	 * case it must match the end or beginning of the pattern exactly. If no
	 * wildcards are defined then a match is true, if the file name contains the
	 * pattern.
	 * 
	 * @param file
	 *            the file whose name is checked
	 ...
	 *            the file
	 * @return the nodeId corresponding to the file. The NodeId is composed of the
	 *         file path in the namespace of the manager.
	 */
	...
	/**
	 * Refreshes the nodes of the folder according to the current files in the disk
	 ...
	 * 
	 * @param file
	 *            the file object corresponding to this folder. The file object is
	 *            not verified, whether it is actually a directory or exists.
	 */
	...
	/**
	 * Define the folder to watch as a string path. The path will be converted to a
	 * file and used to call {@link #setFile(File)}.
	 * 
	 * @param path
	 *            the path to set
	 */
	...
	/**
	 * Define an optional filter which is used to select only files that match the
	 * specific criteria.
	 * 
	 * @param filter
	 *            the filter to set. The filter may contain a comma separated list
	 *            of patterns. See {@link #matchesFilter(File)} for the detailed
	 *            pattern rules.
	 */
		...
		if (!contains(file)) {
			...
			try {
				...
				if (node instanceof FileFolderType) {
					...
					logger.error(
							"A NodeId exists for the folder " + file + " already and it is not of type FileFolderType");
					return false;
				}
			} catch (StatusException e) {
				// does not exist yet
